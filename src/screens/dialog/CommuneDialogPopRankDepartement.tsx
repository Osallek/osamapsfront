...
function CommuneDialogPopRank({ commune, setMaxWidth }: CommuneDialogPopRankProps) {
  ...
  useEffect(() => {
    setDatas(popRankLine([commune.population.departementRanks]));
  }, [commune]);
  ...
  return (
    <DialogPop data={ datas }
               yAxis={ [{ dataKey: 'data0', key: '0', domain: [1, data.departements.departements[commune.departement].communes.length] }] }
               lines={ [{ dataKey: 'data0' }] }
