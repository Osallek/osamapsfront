...
function DepartementDialogPopRank({ departement, setMaxWidth }: DepartementDialogPopRankProps) {
  ...
  useEffect(() => {
    setDatas(popRankLine([departement.population.regionRanks]));
  }, [departement]);
  ...
  return (
    <DialogPop data={ datas }
               yAxis={ [{ dataKey: 'data0', key: '0', domain: [1, Object.values(data.regions.regions[departement.region].departements).length] }] }
               lines={ [{ dataKey: 'data0' }] }
